...
def model_instance_with_best_guess_values_of_uncertainties(slr_scenario: int) -> dict:
    """
    This function initializes and returns a dictionary of input parameters
    for the sea level rise (SLR) model based on the specified SLR scenario.
    ...
    for n in np.arange(1, 5):
        ...
        for i in range(1, n + 1):
            M = M + meanGEV / i
        pars["E"] += 0.1 * p[n - 1] * M  # Annual expected storm erosion
    pars["epsilon"] = 0.05  # Increase in storm erosion with SLR
    """Property value parameters"""
    pars["d"] = 0
    pars["alpha"] = 1.025  # Property value increase due to 1m
    # increase in beach width
    pars["beta"] = 0.975  # Property value decrease due to 1m increase in sea level
    pars["A"] = 669000
    pars["v_init"] = pars["A"] * (pars["alpha"] ** (pars["x_init"]))  # Baseline
    # property value
    pars["W"] = 5 * 10**5  # Non-structural value at risk
    ...
    # $9.55/m^3, 5470 m of beach length, and 224,000 m^3
    # per 6.096 m nourishment
    pars["xi"] = 0.05  # Exponential increase in c2 as time progresses (0 means
    # cost is autonomous)
    pars["constructionCosts"] = 0
    pars["Cfunc"] = "concave"
    pars["phi_exp"] = 5.6999  # Sea level base for proportion damaged
...
def compute_coastal_state_variables(
    ...
    x = term1 + term2
    # print("mu", pars["mu"])
    # print("x_nourished", pars["x_nourished"])
    # print("theta", pars["theta"])
    # print("t", t)
    # print("tau", tau)
    # print("R", R)
    # print("nourishing", nourishing)
    # print("L_t", L_int)
    # print("Erosion", E)
    # print("a", pars["a"])
    # print("b", pars["b"])
    # print("Ti", pars["Ti"])
    # print("epsilon", pars["epsilon"])
